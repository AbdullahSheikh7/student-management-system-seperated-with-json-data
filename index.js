...
const addStudent = async (studentsJSONPath) => {
    let spinner = createSpinner("Please wait...");
    let courses = Array.from(JSON.parse(fs.readFileSync(path.join(process.cwd(), "data/courses.json")).toString()));
    ...
    if (input.confirmAddingStudent) {
        let student = new Student(input.studentName, input.studentClass, input.studentRollNo, input.studentEnrolledCourses);
...
const enrollStudent = async (studentsJSONPath) => {
    let spinner = createSpinner("Please wait...");
    let studentsObject = JSON.parse(fs.readFileSync(studentsJSONPath).toString());
    let studentsNames = Object.keys(studentsObject).map((e) => studentsObject[e].name);
    let courses = JSON.parse(fs.readFileSync(path.join(process.cwd(), "data/courses.json")).toString());
    ...
    if (course.confirmEnrollingStudent) {
        studentsObject[Object.keys(studentsObject)[studentsNames.indexOf(student.student)]].courses = studentsObject[Object.keys(studentsObject)[studentsNames.indexOf(student.student)]].courses.concat(course.courses);
        studentsObject[Object.keys(studentsObject)[studentsNames.indexOf(student.student)]].fees =
            studentsObject[Object.keys(studentsObject)[studentsNames.indexOf(student.student)]].courses.length * 100;
        studentsObject[Object.keys(studentsObject)[studentsNames.indexOf(student.student)]].feePaid = false;
        fs.writeFileSync(studentsJSONPath, JSON.stringify(studentsObject));
        spinner.success({
...
const payTuitionFees = async (studentsJSONPath, balance) => {
    let spinner = createSpinner("Please wait...");
    let studentsObject = JSON.parse(fs.readFileSync(studentsJSONPath).toString());
    let feePaidFalseStudentsIds = Object.keys(studentsObject).filter((e) => !studentsObject[e].feePaid);
    let feePaidFalseStudentsNames = feePaidFalseStudentsIds.map((e) => studentsObject[e].name);
    let studentsNames = Object.keys(studentsObject).map((e) => studentsObject[e].name);
    if (Object.keys(studentsObject).length == 0) {
        console.log(`${chalk.green("?")} No students found\n`);
        return balance;
    ...
    spinner.start();
    studentsObject[Object.keys(studentsObject)[studentsNames.indexOf(payFeesStudent.student)]].feePaid = true;
    fs.writeFileSync(studentsJSONPath, JSON.stringify(studentsObject));
    await sleep();
    spinner.success({
    ...
    return (balance +=
        studentsObject[Object.keys(studentsObject)[studentsNames.indexOf(payFeesStudent.student)]].fees);
};
const showStatus = async (studentsJSONPath) => {
    let spinner = createSpinner("Please wait...");
    let studentsObject = JSON.parse(fs.readFileSync(studentsJSONPath).toString());
    let studentsIds = Object.keys(studentsObject);
...
const main = async () => {
    let exit = false;
    let studentsJSONPath = path.join(process.cwd(), "data/students.json");
    let balance = parseInt(fs.readFileSync(path.join(process.cwd(), "data/balance.json")).toString());
    ...
    do {
        ...
        if (option == "Add student") {
            ...
            fs.writeFileSync(path.join(process.cwd(), "data/balance.json"), balance.toString());
        }
